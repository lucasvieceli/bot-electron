...
import { EVENT_GAME_LOOP_BROWSER, EVENT_GAME_LOOP_STATUS, EVENT_GAME_LOOP_STATUS_PAUSED } from '../../../variables';
...
export class GameLoop {
    ...
    public isPaused: boolean | null = null;
    ...
    async stop(message = true) {
        try {
            ...
            contextBridge.broadcastMessage(EVENT_GAME_LOOP_BROWSER, {data: 0});
    ...
    setExecute(value: boolean) {
        this.execute = value;
        contextBridge.broadcastMessage(EVENT_GAME_LOOP_STATUS, {data: value});
    }
    setPaused(value: boolean | null) {
        this.isPaused = value;
        contextBridge.broadcastMessage(EVENT_GAME_LOOP_STATUS_PAUSED, {data: value});
    }
    ...
    private async getBrowsers() {
        ...
        contextBridge.broadcastMessage(EVENT_GAME_LOOP_BROWSER, {data:accounts.length});
        this.browsers = [];
